...
- Optional Google sign-in for one-click waitlist signup
...
Create a Google OAuth Web application in Google Cloud Console and add the site origins to its authorized JavaScript origins. Set `NEXT_PUBLIC_GOOGLE_CLIENT_ID` for the browser and `GOOGLE_CLIENT_ID` as a server-side Cloudflare Worker secret. Both values must match. Google sign-in verifies the ID token before saving the verified name and email to the waitlist.
